...
from parser10.add_new_product_if_not_exists import add_new_product_if_not_exists
...
async def purchase_product(product_id, price, wallet_balance):
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
        context = await browser.new_context(storage_state='storage_state.json')
        page = await context.new_page()
        quantity = math.floor(wallet_balance / price)
        url = f"https://www.wildberries.ru/catalog/{product_id}/detail.aspx"
        await page.goto(url)
        logging.info(f"Opened product page: {url}")
        await asyncio.sleep(2)
...
async def purchaseCheck(db_url, original_table_name, wallet_balance):
    ...
    if new_product is None:
        ...
        if product_id is None:
            ...
            price_int = int(price)
            await purchase_product(product_id_str, price_int, wallet_balance)
            last_checked_id = product_id
            logging.info(f"Bought new product: {product_id_str}")

    add_new_product_if_not_exists('compared_products_insta_buy', 'compared_products_insta_buy_duplicate_telegram', db_url, product_id_set_telegram)
    session.close()